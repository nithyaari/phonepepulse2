...
def about_page():
    ...
    col1.image(Image.open("C:/Users/smm/phonepe/phonepe image.jpg"), width=500)
    with col1:
        st.subheader(
    ...
    with col2:
        st.video("C:/Users/smm/phonepe/phonepe.mp4")
...
def home_page():
    col1, col2 = st.columns(2)
    with col1:
        st.video("C:/Users/smm/phonepe/phonepe upi.mp4")
    with col2:
        st.title(':violet[PHONEPE PULSE DATA VISUALISATION]')
        st.subheader(':violet[Phonepe Pulse]:')
        st.write(
            'PhonePe Pulse is a feature offered by the Indian digital payments platform called PhonePe.PhonePe Pulse provides users with insights and trends related to their digital transactions and usage patterns on the PhonePe app.')
        st.subheader(':violet[Phonepe Pulse Data Visualisation]:')
        st.write(
            'Data visualization refers to the graphical representation of data using charts, graphs, and other visual elements to facilitate understanding and analysis in a visually appealing manner.'
            'The goal is to extract this data and process it to obtain insights and information that can be visualized in a user-friendly manner.')
        st.markdown("## :violet[Created By] : Nithya ")
        st.markdown("[Inspired from](https://www.phonepe.com/pulse/)")
        st.markdown("[Githublink](https://github.com/nithyaari)")
...
def analysis_page():
    ...
    if select == "INDIA":
        ...
        with tab2:
            ...
            cursor.execute(
                f"SELECT State, SUM(User_Count) FROM agg_user WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}' GROUP BY State;")
            ...
            cursor.execute(
                f"SELECT SUM(User_Count), AVG(User_Count) FROM agg_user WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}';")
    ...
    if select == "STATES":
        ...
        with tab4:
            ...
            # User Analysis Bar chart query
            cursor.execute(
                f"SELECT Quarter, SUM(User_Count) FROM agg_user WHERE State = '{st_us_st}' AND Year = '{st_us_yr}' GROUP BY Quarter;")
            ...
            # Total User Count table query
            cursor.execute(
                f"SELECT SUM(User_Count), AVG(User_Count) FROM agg_user WHERE State = '{st_us_st}' AND Year = '{st_us_yr}';")
...
def insights_page():
    ...
    st.subheader(
        "The basic insights are derived from the Analysis of the Phonepe Pulse data. It provides a clear idea about the analysed data.")
    options = ["--select--",
    ...
    # 3
        ...
        cursor.execute(
            "SELECT DISTINCT State, Pincode, SUM(Registered_users) AS Users FROM top_user GROUP BY State, Pincode ORDER BY Users DESC LIMIT 10;");
        data = cursor.fetchall()
    ...
    # 4
        ...
        cursor.execute(
            "SELECT DISTINCT State, Pincode, SUM(Registered_users) AS Users FROM top_user GROUP BY State, Pincode ORDER BY Users ASC LIMIT 10");
        data = cursor.fetchall()
        columns = ['State', 'Pincode', 'Registered_users']
        df = pd.DataFrame(data, columns=columns, index=range(1, len(data) + 1))
        col1, col2 = st.columns(2)
    ...
    st.markdown(
        "[DOWNLOAD REPORT](https://www.phonepe.com/pulsestatic/732/pulse/static/83bc2c9e9038369af2eb9eb7d62cb49f/PhonePe_Pulse_BCG_report.pdf)")
